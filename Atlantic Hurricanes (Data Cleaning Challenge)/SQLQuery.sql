...
select [Wind speed], 
       replace(substring([Wind speed], 1, charindex('mph',[Wind speed])),'Â m','') as Windspeed_mph,
	   replace(replace(substring([Wind speed], charindex('(',[Wind speed]), charindex('km/h',[Wind speed])),'Â km/h)',''),'(','') as Windspeed_kmph
from Hurricanes

UPDATE T1
SET T1.Windspeed_mph = (select replace(substring([Wind speed], 1, charindex('mph',[Wind speed])),'Â m','')
       from Hurricanes AS T2 WHERE T2.F1 = T1.F1)
FROM Hurricanes AS T1

UPDATE T1
SET T1.Windspeed_kmph = (select replace(replace(substring([Wind speed], charindex('(',[Wind speed]), charindex('km/h',[Wind speed])),'Â km/h)',''),'(','')
       from Hurricanes AS T2 WHERE T2.F1 = T1.F1)
FROM Hurricanes AS T1

select Pressure,
case when CHARINDEX('hPa', Pressure) <> 0 then replace(replace(substring(Pressure, 1, charindex('hPa', Pressure)),'Â h',''),',','')
     else NULL
end as Pressure_hpa,
case when CHARINDEX('hPa', Pressure) <> 0 then replace(replace(substring(Pressure, charindex('(', Pressure),charindex(')', Pressure)),'(',''),'Â inHg)','')
     else NULL
end as Pressure_Hg
from Hurricanes
